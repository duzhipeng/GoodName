...
def getScore(name):
    ...
    """
    try:
        surname = parse.quote(name[0:1].encode('utf8'))
        lastname = parse.quote(name[1:].encode('utf8'))
    except UnicodeEncodeError as e:
        print(name, '出错：', str(e))
        return
    s = parse.quote(SEX.encode('utf8'))
    detail_url = "http://www.qimingzi.net/nameResultpc.aspx?surname=" + surname + "&name=" + lastname + "&sex=" + s
    html = getHtml(detail_url)
    if html is None:
        return
    first_tag = '<div class="fenshu">'
    last_tag = '<a name="zhuanye">'
    score = html[html.index(first_tag) + len(first_tag): html.index(last_tag)-56]
